...
Delete any existing `{{workspace.ephemeral}}/HANDOFF.md`, then create a new one capturing everything the next developer (or session) needs to continue this work.
...
## Rules
- Delete the old handoff first — a new handoff supersedes the prior one for the same `handoff_key`. Write the new file to `{{workspace.ephemeral}}/HANDOFF.md`.
